...
decisionsRouter.post("/communes/:commune/signataires", requirePermission("signataires"), requireRole("mairie", "admin"), async (req: AuthRequest, res) => {
  const commune = decodeURIComponent(String(req.params["commune"] ?? ""));
  const scope = await getCommuneScope(req.user!.id, req.user!.role);
  if (!communeInScope(commune, scope)) {
    return res.status(403).json({ error: "Commune hors de votre périmètre" });
...
decisionsRouter.put("/communes/:commune/signataires/:id", requirePermission("signataires"), requireRole("mairie", "admin"), async (req: AuthRequest, res) => {
  const { id } = req.params as { id: string };
  const commune = decodeURIComponent(String(req.params["commune"] ?? ""));
  const scope = await getCommuneScope(req.user!.id, req.user!.role);
  if (!communeInScope(commune, scope)) {
...
decisionsRouter.delete("/communes/:commune/signataires/:id", requirePermission("signataires"), requireRole("mairie", "admin"), async (req: AuthRequest, res) => {
  const { id } = req.params as { id: string };
  const commune = decodeURIComponent(String(req.params["commune"] ?? ""));
  const scope = await getCommuneScope(req.user!.id, req.user!.role);
  if (!communeInScope(commune, scope)) {
