...
| `dossier_piece_annotations` | `dossierPieceAnnotations.ts` | **Calque d'annotation vectorielle des pièces citoyen** (internalise Inkscape/Foxit). 1 ligne = 1 marque | `dossier_id` / `piece_id` FK cascade · `page` · `tool` (ellipse/rect/arrow/freehand/text/polygon) · `geometry` jsonb (coords en % de page) · `style` jsonb (couleur, épaisseur) · `comment` · **`visibility`** (interne/citoyen) · `author_user_id`. À ne pas confondre avec `document_segment_annotations` (docs réglementaires → IA) |
