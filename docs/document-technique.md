...
| `dossier_messages` | `dossier_messages.ts` | Fils citoyen↔mairie + mairie↔service | `dossier_id` FK · `consultation_id` FK (scope service) · `from_role` (citoyen/mairie/service) · `content` html · `parent_id` (réponses imbriquées) · `mentions` jsonb · **`attachments` jsonb** (références GED `dossier_documents` : `{document_id, nom, url, type}[]`) · `read_at` |
...
| `dossier_courriers` | `dossierCourriers.ts` | Courriers d'instruction émis (snapshot figé) | `type` (pieces_complementaires / refus / non_opposition / majoration_delai / notification_decision / general) · `body_snapshot` html · `pieces_jointes_ids` jsonb · `articles_cites` jsonb · **`attachments` jsonb** (références GED `dossier_documents`) · `delivery_method` (print/email/ar) |
...
| `dossier_documents` | `dossierDocuments.ts` | **GED du dossier** — coffre des documents *produits* par l'instruction (export d'une pièce annotée ; demain courriers PDF). Distinct de `dossier_pieces_jointes` (pièces *déposées* par le citoyen) | `dossier_id` FK cascade · `nom` · `url` (clé /api/uploads) · `type` MIME · `taille` · `category` (annotation/courrier/autre) · `source_piece_id` FK (pièce d'origine) · **`shared_with_citizen` bool** (garde de confidentialité : ouvert au citoyen seulement une fois joint à un envoi) · `created_by` |
| `dossier_piece_annotations` | `dossierPieceAnnotations.ts` | **Calque d'annotation vectorielle des pièces citoyen** (internalise Inkscape/Foxit). 1 ligne = 1 marque | `dossier_id` / `piece_id` FK cascade · `page` · `tool` (ellipse/rect/arrow/freehand/text/**scale**/**measure**/polygon) · `geometry` jsonb (coords en % de page) · `style` jsonb (couleur, épaisseur, **`meters`** pour l'échelle) · `comment` · **`visibility`** (interne/citoyen) · `author_user_id`. Échelle : `tool="scale"` + `style.meters` calibre le ratio m/px de la page → `measure` (distances) et surface des rectangles. À ne pas confondre avec `document_segment_annotations` (docs réglementaires → IA) |
